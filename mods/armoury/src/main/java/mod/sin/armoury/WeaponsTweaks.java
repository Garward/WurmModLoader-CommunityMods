...
public class WeaponsTweaks {
	...
	private static double newGetMaterialDamageBonus(byte material){
    ...
    private static float newGetBaseSpeedForWeapon(Item weapon){
    ...
    private static float newGetMaterialParryBonus(byte material){
    ...
    private static double newGetMaterialArmourDamageBonus(byte material){
    ...
    public static void applyWeaponMaterialStat(WeaponStatQueryEvent event){
        switch (event.getStatType()) {
            case DAMAGE: {
                Double modifier = materialWeaponDamage.get(event.getMaterial());
                if (modifier != null) {
                    event.setValue(modifier);
                }
                break;
            }
            case SPEED: {
                ...
                event.setValue(newGetBaseSpeedForWeapon(item));
                break;
            }
            case PARRY: {
                Float parry = materialWeaponParry.get(event.getMaterial());
                if (parry != null) {
                    event.setValue(parry);
                }
                break;
            }
            case ARMOUR_DAMAGE: {
                Double armour = materialWeaponArmourDamage.get(event.getMaterial());
                if (armour != null) {
                    event.setValue(armour);
                }
                break;
            }
            default:
