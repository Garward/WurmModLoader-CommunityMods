...
tasks.register<Jar>("modJar") {
    ...
    archiveBaseName.set("cropmod")
    archiveVersion.set("")
    destinationDirectory.set(file("${projectDir}/dist/mods/cropmod"))
}
...
tasks.register<Copy>("modDistribution") {
    dependsOn("modJar")
    from("src/dist")
    into("${projectDir}/dist/mods/cropmod")
}
...
tasks.register<Copy>("deployMod") {
    dependsOn("modDistribution")
    val wurmServerDir: String by rootProject.extra
    from(layout.projectDirectory.dir("dist/mods/cropmod"))
    into("$wurmServerDir/mods/cropmod")
}
