...
tasks.register<Jar>("modJar") {
    ...
    archiveBaseName.set("gemaugmentation")
    archiveVersion.set("")
    destinationDirectory.set(file("${projectDir}/dist/mods/gemaugmentation"))
}

tasks.register<Copy>("modDistribution") {
    dependsOn("modJar")
    from("src/dist")
    into("${projectDir}/dist/mods/gemaugmentation")
}
...
tasks.register<Copy>("deployMod") {
    dependsOn("modDistribution")
    val wurmServerDir: String by rootProject.extra
    from(layout.projectDirectory.dir("dist/mods/gemaugmentation"))
    into("$wurmServerDir/mods/gemaugmentation")
}
