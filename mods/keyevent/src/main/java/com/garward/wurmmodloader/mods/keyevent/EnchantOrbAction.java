...
/**
 * Activate an enchant orb on a target item to transfer all enchants from
 * the orb onto the item. Each enchant respects target preconditions and
 * stacking caps; runes obey the "one rune per item" rule. Direct port of
 * {@code mod.sin.actions.items.EnchantOrbAction}.
 */
public class EnchantOrbAction implements ModAction {

    private static final Logger logger = Logger.getLogger(EnchantOrbAction.class.getName());
        ...
        return new BehaviourProvider() {
            ...
            public List<ActionEntry> getBehavioursFor(Creature performer, Item source, Item object) {
                if (performer instanceof Player && source != null && object != null
                        && source.getTemplateId() == KeyEventTemplates.enchantOrbTemplateId
                        && source != object) {
        ...
        return new ActionPerformer() {
            ...
            public boolean action(Action act, Creature performer, Item source, Item target,
                ...
                if (!(performer instanceof Player)) {
                    logger.info("[keyevent] non-player attempted EnchantOrb transfer");
                    return true;
                }
                Player player = (Player) performer;
                if (source.getTemplate().getTemplateId() != KeyEventTemplates.enchantOrbTemplateId) {
                    player.getCommunicator().sendNormalServerMessage(
                            "You must use an Enchant Orb to transfer enchants.");
                    return true;
                }
                if (target.getTemplate().getTemplateId() == KeyEventTemplates.enchantOrbTemplateId) {
                    player.getCommunicator().sendNormalServerMessage(
                            "You cannot enchant an Enchant Orb with another.");
                    return true;
                }
                ItemSpellEffects effs = source.getSpellEffects();
                if (effs == null || effs.getEffects().length == 0) {
                    player.getCommunicator().sendNormalServerMessage(
                            "The " + source.getTemplate().getName() + " has no enchants.");
                    return true;
                }
                    ...
                    if (spell == null) {
                        if (eff.type < -60) {
                            ...
                            if (teffs.getNumberOfRuneEffects() > 0) {
                                ...
                                player.getCommunicator().sendAlertServerMessage(
                                        "The " + target.getTemplate().getName()
                                                + " already has a rune attached and resists the application of the "
                                                + eff.getName() + ".");
                                continue;
                            } else {
                                canEnchant = true;
                            ...
                            if (teffs.getSpellEffect(type) != null) {
                                ...
                                if (power >= 100f) {
                                    player.getCommunicator().sendAlertServerMessage(
                                            "The " + target.getTemplate().getName()
                                                    + " already has the maximum power for "
                                                    + eff.getName() + ", and refuses to accept more.");
                                    continue;
                                    ...
                                    player.getCommunicator().sendSafeServerMessage(
                                            "The " + eff.getName() + " transfers some of its power to the "
                                                    + target.getTemplate().getName() + ".");
                                    continue;
                                } else {
                                    teffs.getSpellEffect(type).setPower(
                                            effs.getSpellEffect(type).getPower() + power);
                                    effs.removeSpellEffect(type);
                                    player.getCommunicator().sendSafeServerMessage(
                                            "The " + eff.getName() + " fully transfers to the "
                                                    + target.getTemplate().getName() + ".");
                                    continue;
                                }
                            } else {
                        ...
                        try {
                            ...
                            if (spell instanceof ItemEnchantment) {
                                m = ItemEnchantment.class.getDeclaredMethod(
                                        "precondition", Skill.class, Creature.class, Item.class);
                            } else {
                                m = spell.getClass().getDeclaredMethod(
                                        "precondition", Skill.class, Creature.class, Item.class);
                            }
                            canEnchant = ReflectionUtil.callPrivateMethod(
                                    spell, m, player.getChannelingSkill(), performer, target);
                        } catch (IllegalAccessException | IllegalArgumentException
                                | InvocationTargetException | NoSuchMethodException e) {
                            logger.warning("[keyevent] precondition reflection failed: " + e.getMessage());
                        }
                    }
                    if (canEnchant) {
                        if (teffs.getSpellEffect(type) != null) {
                            if (teffs.getSpellEffect(type).getPower() >= eff.getPower()) {
                                player.getCommunicator().sendAlertServerMessage(
                                        "The " + target.getTemplate().getName()
                                                + " already has a more powerful " + eff.getName()
                                                + " and resists the transfer.");
                                continue;
                            } else {
                                teffs.getSpellEffect(type).setPower(eff.getPower());
                                effs.removeSpellEffect(type);
                                player.getCommunicator().sendSafeServerMessage(
                                        "The " + eff.getName() + " replaces the existing enchant.");
                                continue;
                            }
                        }
                        ...
                        effs.removeSpellEffect(type);
                        player.getCommunicator().sendSafeServerMessage(
                                "The " + eff.getName() + " transfers to the "
                                        + target.getTemplate().getName() + ".");
                    }
                }
                if (effs.getEffects().length == 0) {
                    player.getCommunicator().sendSafeServerMessage(
                            "The " + source.getTemplate().getName()
                                    + " exhausts the last of its magic and vanishes.");
