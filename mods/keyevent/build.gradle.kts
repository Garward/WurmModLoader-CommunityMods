...
tasks.register<Jar>("modJar") {
    ...
    archiveBaseName.set("keyevent")
    archiveVersion.set("")
    destinationDirectory.set(file("${projectDir}/dist/mods/keyevent"))
}

tasks.register<Copy>("modDistribution") {
    dependsOn("modJar")
    from("src/dist")
    into("${projectDir}/dist/mods/keyevent")
}
...
tasks.register<Copy>("deployMod") {
    dependsOn("modDistribution")
    val wurmServerDir: String by rootProject.extra
    from(layout.projectDirectory.dir("dist/mods/keyevent"))
    into("$wurmServerDir/mods/keyevent")
}
