...
tasks.register<Jar>("modJar") {
    ...
    archiveBaseName.set("achievementchanges")
    archiveVersion.set("")
    destinationDirectory.set(file("${projectDir}/dist/mods/achievementchanges"))
}

tasks.register<Copy>("modDistribution") {
    dependsOn("modJar")
    from("src/dist")
    into("${projectDir}/dist/mods/achievementchanges")
}
...
tasks.register<Copy>("deployMod") {
    dependsOn("modDistribution")
    val wurmServerDir: String by rootProject.extra
    from(layout.projectDirectory.dir("dist/mods/achievementchanges"))
    into("$wurmServerDir/mods/achievementchanges")
}
