...
tasks.register<Jar>("modJar") {
    ...
    archiveBaseName.set("spellcraft")
    archiveVersion.set("")
    destinationDirectory.set(file("${projectDir}/dist/mods/spellcraft"))
}
...
tasks.register<Copy>("modDistribution") {
    dependsOn("modJar")
    from("src/dist")
    into("${projectDir}/dist/mods/spellcraft")
}
...
tasks.register<Copy>("deployMod") {
    dependsOn("modDistribution")
    val wurmServerDir: String by rootProject.extra
    from(layout.projectDirectory.dir("dist/mods/spellcraft"))
    into("$wurmServerDir/mods/spellcraft")
}
