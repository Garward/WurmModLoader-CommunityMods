...
tasks.register<Jar>("modJar") {
    ...
    archiveBaseName.set("mission")
    archiveVersion.set("")
    destinationDirectory.set(file("${projectDir}/dist/mods/mission"))
}

tasks.register<Copy>("modDistribution") {
    dependsOn("modJar")
    from("src/dist")
    into("${projectDir}/dist/mods/mission")
}
...
tasks.register<Copy>("deployMod") {
    dependsOn("modDistribution")
    val wurmServerDir: String by rootProject.extra
    from(layout.projectDirectory.dir("dist/mods/mission"))
    into("$wurmServerDir/mods/mission")
}
