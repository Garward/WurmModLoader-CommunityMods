...
tasks.register<Jar>("modJar") {
    ...
    archiveBaseName.set("customtitles")
    archiveVersion.set("")
    destinationDirectory.set(file("${projectDir}/dist/mods/customtitles"))
}

tasks.register<Copy>("modDistribution") {
    dependsOn("modJar")
    from("src/dist")
    into("${projectDir}/dist/mods/customtitles")
}
...
tasks.register<Copy>("deployMod") {
    dependsOn("modDistribution")
    val wurmServerDir: String by rootProject.extra
    from(layout.projectDirectory.dir("dist/mods/customtitles"))
    into("$wurmServerDir/mods/customtitles")
}
