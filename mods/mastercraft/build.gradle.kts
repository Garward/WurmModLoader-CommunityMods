...
tasks.register<Jar>("modJar") {
    ...
    archiveBaseName.set("mastercraft")
    archiveVersion.set("")
    destinationDirectory.set(file("${projectDir}/dist/mods/mastercraft"))
}

tasks.register<Copy>("modDistribution") {
    dependsOn("modJar")
    from("src/dist")
    into("${projectDir}/dist/mods/mastercraft")
}
...
tasks.register<Copy>("deployMod") {
    dependsOn("modDistribution")
    val wurmServerDir: String by rootProject.extra
    from(layout.projectDirectory.dir("dist/mods/mastercraft"))
    into("$wurmServerDir/mods/mastercraft")
}
