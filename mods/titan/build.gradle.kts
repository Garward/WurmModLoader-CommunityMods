...
tasks.register<Jar>("modJar") {
    ...
    archiveBaseName.set("titan")
    archiveVersion.set("")
    destinationDirectory.set(file("${projectDir}/dist/mods/titan"))
}

tasks.register<Copy>("modDistribution") {
    dependsOn("modJar")
    from("src/dist")
    into("${projectDir}/dist/mods/titan")
}
...
tasks.register<Copy>("deployMod") {
    dependsOn("modDistribution")
    val wurmServerDir: String by rootProject.extra
    from(layout.projectDirectory.dir("dist/mods/titan"))
    into("$wurmServerDir/mods/titan")
}
