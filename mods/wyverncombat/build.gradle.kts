...
tasks.register<Jar>("modJar") {
    ...
    archiveBaseName.set("wyverncombat")
    archiveVersion.set("")
    destinationDirectory.set(file("${projectDir}/dist/mods/wyverncombat"))
}

tasks.register<Copy>("modDistribution") {
    dependsOn("modJar")
    from("src/dist")
    into("${projectDir}/dist/mods/wyverncombat")
}
...
tasks.register<Copy>("deployMod") {
    dependsOn("modDistribution")
    val wurmServerDir: String by rootProject.extra
    from(layout.projectDirectory.dir("dist/mods/wyverncombat"))
    into("$wurmServerDir/mods/wyverncombat")
}
