...
tasks.register<Jar>("modJar") {
    ...
    archiveBaseName.set("supplydepot")
    archiveVersion.set("")
    destinationDirectory.set(file("${projectDir}/dist/mods/supplydepot"))
}

tasks.register<Copy>("modDistribution") {
    dependsOn("modJar")
    from("src/dist")
    into("${projectDir}/dist/mods/supplydepot")
}
...
tasks.register<Copy>("deployMod") {
    dependsOn("modDistribution")
    val wurmServerDir: String by rootProject.extra
    from(layout.projectDirectory.dir("dist/mods/supplydepot"))
    into("$wurmServerDir/mods/supplydepot")
}
