...
public class CustomCreature implements ModCreature {
    ...
    // Vehicle options
    private int vehicleNumSeats = -1;
    private float vehicleSkillNeeded = 20.0f;
    private String vehicleName = null;
    private float vehicleMaxDepthMeters = -0.7f;
    private float vehicleMaxHeightDiff = 0.04f;
    private float vehicleMaxSpeed = 25.0f;
    private boolean vehicleCanHaveEquipment = true;
    private byte vehicleCommandType = (byte) 3;

    private ArrayList<Integer> vehicleSeatFightModifiers = new ArrayList<>();
    private ArrayList<Float> vehicleSeatFightModifiersCovers = new ArrayList<>();
    private ArrayList<Float> vehicleSeatFightModifiersManoeuvre = new ArrayList<>();

    private ArrayList<Integer> vehicleSeatOffsets = new ArrayList<>();
    private ArrayList<Float> vehicleSeatOffsetsX = new ArrayList<>();
    private ArrayList<Float> vehicleSeatOffsetsY = new ArrayList<>();
    private ArrayList<Float> vehicleSeatOffsetsZ = new ArrayList<>();

    public CustomCreature(Properties properties, String uniqueId) {
        this.p = properties;
    ...
    public CreatureTemplateBuilder createCreateTemplateBuilder() {
        return new CreatureTemplateBuilder(prePrefix+uniqueId) {
            @Override
            public CreatureTemplate build() {

                CreatureTemplate temp = new CreatureTemplateBuilder(prePrefix+uniqueId, getStringProperty("name"), "A creature.", "model.creature.humanoid.rooster.brown", new int[0], (byte) 3, (short) 5, (byte) 0, (short) 60, (short) 30, (short) 90, "sound.death.lion",
    ...
    private CreatureTemplate buildOnTemplate(CreatureTemplate temp) {
        logger.info("Creating creature template for: "+uniqueId);

        boolean logAddedCreatureProperties = CustomCreatures.getInstance().isLogAddedCreatureProperties();

        for(Object propertyObject : p.keySet()){
            ...
            switch(property){
                ...
                case "types":
                    try {
                        Object types = getIntArrayProperty("types");
                        ReflectionUtil.callPrivateMethod(temp, ReflectionUtil.getMethod(temp.getClass(), "assignTypes"), types);
                    } catch (NoSuchMethodException | IllegalAccessException | InvocationTargetException e){
                        throw new HookException(e);
                    }
                    break;
                case "vehicleNumSeats":
                    vehicleNumSeats = getIntProperty("vehicleNumSeats") - 1;
                    break;
                case "vehicleSkillNeeded":
                    vehicleSkillNeeded = getFloatProperty("vehicleSkillNeeded");
                    break;
                case "vehicleName":
                    vehicleName = getStringProperty("vehicleName");
                    break;
                case "vehicleMaxDepthMeters":
                    vehicleMaxDepthMeters = getFloatProperty("vehicleMaxDepthMeters");
                    break;
                case "vehicleMaxHeightDiff":
                    vehicleMaxHeightDiff = getFloatProperty("vehicleMaxHeightDiff");
                    break;
                case "vehicleMaxSpeed":
                    vehicleMaxSpeed = getFloatProperty("vehicleMaxSpeed");
                    break;
                case "vehicleCanHaveEquipment":
                    vehicleCanHaveEquipment = getBooleanProperty("vehicleCanHaveEquipment");
                    break;
                case "vehicleCommandType":
                    vehicleCommandType = getByteProperty("vehicleCommandType");
                    break;
                case "disabled":
                case "copy":
                case "override":
                ...
                default:
                    if(!property.startsWith("skill")
                            ...
                            && !property.startsWith("vehicleSeatOffset")
                            && !property.startsWith("vehicleSeatFightModifier"))
                        throw new HookException("No such property \""+property+"\" in "+uniqueId);
            }
            if(logAddedCreatureProperties)
        ...
        try{
            for(int i = 1;;++i) {
                String[] values = getStringArrayProperty("vehicleSeatFightModifier"+i, 3);
                vehicleSeatFightModifiers.add(Integer.parseInt(values[0]));
                vehicleSeatFightModifiersCovers.add(Float.parseFloat(values[1]));
                vehicleSeatFightModifiersManoeuvre.add(Float.parseFloat(values[2]));
                if(logAddedCreatureProperties)
                    logger.info("\tAdded vehicleSeatFightModifier"+i);
            }
        } catch (MissingPropertyException ignored){}

        try{
            for(int i = 1;;++i) {
                String[] values = getStringArrayProperty("vehicleSeatOffset"+i, 4);
                vehicleSeatOffsets.add(Integer.parseInt(values[0]));
                vehicleSeatOffsetsX.add(Float.parseFloat(values[1]));
                vehicleSeatOffsetsY.add(Float.parseFloat(values[2]));
                vehicleSeatOffsetsZ.add(Float.parseFloat(values[3]));
                if(logAddedCreatureProperties)
                    logger.info("\tAdded vehicleSeatOffset"+i);
            }
        } catch (MissingPropertyException ignored){}
    ...
    private String getStringProperty(String propName) throws MissingPropertyException {
        ...
        if(value == null)
            throw new MissingPropertyException("Property \""+propName+"\" not found for "+uniqueId);
        return value.trim();
    }
    ...
    public ModVehicleBehaviour getVehicleBehaviour() {
        if(vehicleNumSeats < 0) return null;

        return new ModVehicleBehaviour() {
            @Override
            public void setSettingsForVehicle(Item item, Vehicle v) { }
            @Override
            public void setSettingsForVehicle(Creature creature, Vehicle v) {
                final VehicleFacade vehicle = wrap(v);
                vehicle.setCreature(true);
                vehicle.createPassengerSeats(vehicleNumSeats);
                vehicle.setSkillNeeded(vehicleSkillNeeded);
                vehicle.setName(vehicleName == null ? creature.getName() : vehicleName);
                vehicle.setMaxDepth(vehicleMaxDepthMeters);
                vehicle.setMaxHeightDiff(vehicleMaxHeightDiff);
                vehicle.setMaxSpeed(vehicleMaxSpeed);
                vehicle.setCanHaveEquipment(vehicleCanHaveEquipment);
                vehicle.setCommandType(vehicleCommandType);

                for(int i = 0; i < vehicleSeatFightModifiers.size(); ++i) {
                    vehicle.setSeatFightMod(vehicleSeatFightModifiers.get(i), vehicleSeatFightModifiersCovers.get(i), vehicleSeatFightModifiersManoeuvre.get(i));
                }
                for(int i = 0; i < vehicleSeatOffsets.size(); ++i) {
                    vehicle.setSeatOffset(vehicleSeatOffsets.get(i), vehicleSeatOffsetsX.get(i), vehicleSeatOffsetsY.get(i), vehicleSeatOffsetsZ.get(i));
                }
                // nullify lists to save memory
                vehicleSeatFightModifiers = null;
                vehicleSeatFightModifiersCovers = null;
                vehicleSeatFightModifiersManoeuvre = null;
                vehicleSeatOffsets = null;
                vehicleSeatOffsetsX = null;
                vehicleSeatOffsetsY = null;
                vehicleSeatOffsetsZ = null;
            }
        };
    }
