...
tasks.register<Jar>("modJar") {
    ...
    archiveBaseName.set("mounted")
    archiveVersion.set("")
    destinationDirectory.set(file("${projectDir}/dist/mods/mounted"))
}

tasks.register<Copy>("modDistribution") {
    dependsOn("modJar")
    from("src/dist")
    into("${projectDir}/dist/mods/mounted")
}
...
tasks.register<Copy>("deployMod") {
    dependsOn("modDistribution")
    val wurmServerDir: String by rootProject.extra
    from(layout.projectDirectory.dir("dist/mods/mounted"))
    into("$wurmServerDir/mods/mounted")
}
