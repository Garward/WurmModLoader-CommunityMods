...
tasks.register<Jar>("modJar") {
    ...
    archiveBaseName.set("crystals")
    archiveVersion.set("")
    destinationDirectory.set(file("${projectDir}/dist/mods/crystals"))
}

tasks.register<Copy>("modDistribution") {
    dependsOn("modJar")
    from("src/dist")
    into("${projectDir}/dist/mods/crystals")
}
...
tasks.register<Copy>("deployMod") {
    dependsOn("modDistribution")
    val wurmServerDir: String by rootProject.extra
    from(layout.projectDirectory.dir("dist/mods/crystals"))
    into("$wurmServerDir/mods/crystals")
}
