...
public class DamageMethods {
    ...
    public static double getDamage(DUSKombat ch, Creature attacker, Item weapon, Creature opponent) {
        ...
        // ========================================================================
        // DUSKombat API: Fire damage calculation event for other mods to modify
        // ========================================================================
        ModActionEvent damageEvent = new ModActionEvent("duskombat:calculate_damage");
        damageEvent.set("attackerId", attacker.getWurmId());
        damageEvent.set("defenderId", opponent.getWurmId());
        damageEvent.set("baseDamage", (int) damage);
        damageEvent.set("damageType", getDamageType(weapon));
        damageEvent.set("weaponId", weapon.getWurmId());
        damageEvent.set("isBackstab", attacker.isStealth() && attacker.opponent != null && !attacker.isVisibleTo(opponent));

        // Default values that mods can modify
        damageEvent.set("damageMultiplier", 1.0);
        damageEvent.set("bonusDamage", 0);

        EventBus.getInstance().post(damageEvent);

        // Apply modifications from other mods
        if (damageEvent.isHandled() && !damageEvent.isCancelled()) {
            double damageMultiplier = (Double) damageEvent.get("damageMultiplier");
            int bonusDamage = (Integer) damageEvent.get("bonusDamage");

            damage = damage * damageMultiplier + bonusDamage;

            //logger.info(String.format("Damage modified by events: multiplier=%.2f, bonus=%d, final=%.2f",
            //    damageMultiplier, bonusDamage, damage));
        }
    ...
    /**
     * Get the damage type name for API events.
     */
    private static String getDamageType(Item weapon) {
        if (weapon.isWeaponSlash()) return "SLASH";
        if (weapon.isWeaponPierce()) return "PIERCE";
        if (weapon.isWeaponCrush()) return "CRUSH";
        if (weapon.isBodyPart()) return "UNARMED";
        return "OTHER";
    }
}
