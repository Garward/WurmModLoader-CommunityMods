...
public class ItemInfo {
    ...
    public static void handleExamine(Communicator comm, Item target) {
        ...
        if(performer != null && performer.getPower() >= 5 || DUSKombatMod.showItemCombatInformation){
            ...
            // ========================================================================
            // DUSKombat API: Fire tooltip event for other mods to add custom lines
            // ========================================================================
            ModQueryEvent tooltipEvent = new ModQueryEvent("duskombat:item_tooltip");
            tooltipEvent.set("itemId", target.getWurmId());
            tooltipEvent.set("playerId", performer.getWurmId());
            tooltipEvent.set("isWeapon", target.isWeapon());
            tooltipEvent.set("isArmour", target.isArmour());

            // Provide damage data for weapons
            if (target.isWeapon()) {
                double baseDamage = DamageMethods.getBaseWeaponDamage(performer, null, target, true);
                tooltipEvent.set("baseDamage", (int) baseDamage);
                tooltipEvent.set("damageType", getDamageType(target));
            }

            EventBus.getInstance().post(tooltipEvent);

            // Display custom tooltip lines from other mods
            if (tooltipEvent.isHandled() && tooltipEvent.has("tooltipLines")) {
                @SuppressWarnings("unchecked")
                List<String> tooltipLines = (List<String>) tooltipEvent.get("tooltipLines");
                if (tooltipLines != null && !tooltipLines.isEmpty()) {
                    for (String line : tooltipLines) {
                        ArrayList<MulticolorLineSegment> customSegments = new ArrayList<>();
                        customSegments.add(new MulticolorLineSegment(line, COLOR_YELLOW));
                        comm.sendColoredMessageEvent(customSegments);
                    }
                }
            }
        }
