...
tasks.register<Jar>("modJar") {
    ...
    archiveBaseName.set("skill")
    archiveVersion.set("")
    destinationDirectory.set(file("${projectDir}/dist/mods/skill"))
}

tasks.register<Copy>("modDistribution") {
    dependsOn("modJar")
    from("src/dist")
    into("${projectDir}/dist/mods/skill")
}
...
tasks.register<Copy>("deployMod") {
    dependsOn("modDistribution")
    val wurmServerDir: String by rootProject.extra
    from(layout.projectDirectory.dir("dist/mods/skill"))
    into("$wurmServerDir/mods/skill")
}
