...
tasks.register<Jar>("modJar") {
    ...
    archiveBaseName.set("bestiary")
    archiveVersion.set("")
    destinationDirectory.set(file("${projectDir}/dist/mods/bestiary"))
}

tasks.register<Copy>("modDistribution") {
    dependsOn("modJar")
    from("src/dist")
    into("${projectDir}/dist/mods/bestiary")
}
...
tasks.register<Copy>("deployMod") {
    dependsOn("modDistribution")
    val wurmServerDir: String by rootProject.extra
    from(layout.projectDirectory.dir("dist/mods/bestiary"))
    into("$wurmServerDir/mods/bestiary")
}
