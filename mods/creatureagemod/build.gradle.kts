...
tasks.register<Jar>("modJar") {
    ...
    archiveBaseName.set("creatureagemod")
    archiveVersion.set("")
    destinationDirectory.set(file("${projectDir}/dist/mods/creatureagemod"))
}
...
tasks.register<Copy>("modDistribution") {
    dependsOn("modJar")
    from("src/dist")
    into("${projectDir}/dist/mods/creatureagemod")
}
...
tasks.register<Copy>("deployMod") {
    dependsOn("modDistribution")
    val wurmServerDir: String by rootProject.extra
    from(layout.projectDirectory.dir("dist/mods/creatureagemod"))
    into("$wurmServerDir/mods/creatureagemod")
}
