...
tasks.register<Jar>("modJar") {
    ...
    archiveBaseName.set("anticheat")
    archiveVersion.set("")
    destinationDirectory.set(file("${projectDir}/dist/mods/anticheat"))
}

tasks.register<Copy>("modDistribution") {
    dependsOn("modJar")
    from("src/dist")
    into("${projectDir}/dist/mods/anticheat")
}
...
tasks.register<Copy>("deployMod") {
    dependsOn("modDistribution")
    val wurmServerDir: String by rootProject.extra
    from(layout.projectDirectory.dir("dist/mods/anticheat"))
    into("$wurmServerDir/mods/anticheat")
}
