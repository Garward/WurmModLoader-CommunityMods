...
tasks.register<Jar>("modJar") {
    ...
    archiveBaseName.set("meditation")
    archiveVersion.set("")
    destinationDirectory.set(file("${projectDir}/dist/mods/meditation"))
}

tasks.register<Copy>("modDistribution") {
    dependsOn("modJar")
    from("src/dist")
    into("${projectDir}/dist/mods/meditation")
}
...
tasks.register<Copy>("deployMod") {
    dependsOn("modDistribution")
    val wurmServerDir: String by rootProject.extra
    from(layout.projectDirectory.dir("dist/mods/meditation"))
    into("$wurmServerDir/mods/meditation")
}
