...
tasks.register<Jar>("modJar") {
    ...
    archiveBaseName.set("soulstealing")
    archiveVersion.set("")
    destinationDirectory.set(file("${projectDir}/dist/mods/soulstealing"))
}

tasks.register<Copy>("modDistribution") {
    dependsOn("modJar")
    from("src/dist")
    into("${projectDir}/dist/mods/soulstealing")
}
...
tasks.register<Copy>("deployMod") {
    dependsOn("modDistribution")
    val wurmServerDir: String by rootProject.extra
    from(layout.projectDirectory.dir("dist/mods/soulstealing"))
    into("$wurmServerDir/mods/soulstealing")
}
