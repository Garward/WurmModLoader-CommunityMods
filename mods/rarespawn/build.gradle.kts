...
tasks.register<Jar>("modJar") {
    ...
    archiveBaseName.set("rarespawn")
    archiveVersion.set("")
    destinationDirectory.set(file("${projectDir}/dist/mods/rarespawn"))
}

tasks.register<Copy>("modDistribution") {
    dependsOn("modJar")
    from("src/dist")
    into("${projectDir}/dist/mods/rarespawn")
}
...
tasks.register<Copy>("deployMod") {
    dependsOn("modDistribution")
    val wurmServerDir: String by rootProject.extra
    from(layout.projectDirectory.dir("dist/mods/rarespawn"))
    into("$wurmServerDir/mods/rarespawn")
}
