...
tasks.register<Jar>("modJar") {
    ...
    archiveBaseName.set("qualityoflife")
    archiveVersion.set("")
    destinationDirectory.set(file("${projectDir}/dist/mods/qualityoflife"))
}

tasks.register<Copy>("modDistribution") {
    dependsOn("modJar")
    from("src/dist")
    into("${projectDir}/dist/mods/qualityoflife")
}
...
tasks.register<Copy>("deployMod") {
    dependsOn("modDistribution")
    val wurmServerDir: String by rootProject.extra
    from(layout.projectDirectory.dir("dist/mods/qualityoflife"))
    into("$wurmServerDir/mods/qualityoflife")
}
