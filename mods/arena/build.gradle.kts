...
tasks.register<Jar>("modJar") {
    ...
    archiveVersion.set("")
    destinationDirectory.set(file("${projectDir}/dist/mods/arena"))
}

tasks.register<Copy>("modDistribution") {
    dependsOn("modJar")
    from("src/dist")
    into("${projectDir}/dist/mods/arena")
}
...
tasks.register<Copy>("deployMod") {
    dependsOn("modDistribution")
    val wurmServerDir: String by rootProject.extra
    from(layout.projectDirectory.dir("dist/mods/arena"))
    into("$wurmServerDir/mods/arena")
}
