...
tasks.register<Jar>("modJar") {
    ...
    archiveBaseName.set("teleport")
    archiveVersion.set("")
    destinationDirectory.set(file("${projectDir}/dist/mods/teleport"))
}

tasks.register<Copy>("modDistribution") {
    dependsOn("modJar")
    from("src/dist")
    into("${projectDir}/dist/mods/teleport")
}
...
tasks.register<Copy>("deployMod") {
    dependsOn("modDistribution")
    val wurmServerDir: String by rootProject.extra
    from(layout.projectDirectory.dir("dist/mods/teleport"))
    into("$wurmServerDir/mods/teleport")
}
