...
tasks.register<Jar>("modJar") {
    ...
    archiveBaseName.set("treasurechest")
    archiveVersion.set("")
    destinationDirectory.set(file("${projectDir}/dist/mods/treasurechest"))
}

tasks.register<Copy>("modDistribution") {
    dependsOn("modJar")
    from("src/dist")
    into("${projectDir}/dist/mods/treasurechest")
}
...
tasks.register<Copy>("deployMod") {
    dependsOn("modDistribution")
    val wurmServerDir: String by rootProject.extra
    from(layout.projectDirectory.dir("dist/mods/treasurechest"))
    into("$wurmServerDir/mods/treasurechest")
}
