...
tasks.register<Jar>("modJar") {
    ...
    archiveBaseName.set("customdeity")
    archiveVersion.set("")
    destinationDirectory.set(file("${projectDir}/dist/mods/customdeity"))
}

tasks.register<Copy>("modDistribution") {
    dependsOn("modJar")
    from("src/dist")
    into("${projectDir}/dist/mods/customdeity")
}
...
tasks.register<Copy>("deployMod") {
    dependsOn("modDistribution")
    val wurmServerDir: String by rootProject.extra
    from(layout.projectDirectory.dir("dist/mods/customdeity"))
    into("$wurmServerDir/mods/customdeity")
}
