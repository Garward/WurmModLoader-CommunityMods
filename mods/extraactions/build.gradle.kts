...
tasks.register<Jar>("modJar") {
    ...
    archiveBaseName.set("extraactions")
    archiveVersion.set("")
    destinationDirectory.set(file("${projectDir}/dist/mods/extraactions"))
}

tasks.register<Copy>("modDistribution") {
    dependsOn("modJar")
    from("src/dist")
    into("${projectDir}/dist/mods/extraactions")
}
...
tasks.register<Copy>("deployMod") {
    dependsOn("modDistribution")
    val wurmServerDir: String by rootProject.extra
    from(layout.projectDirectory.dir("dist/mods/extraactions"))
    into("$wurmServerDir/mods/extraactions")
}
