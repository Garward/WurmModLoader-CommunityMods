...
class SmoothTerrainPerformer implements ActionPerformer {

    private static final Logger logger = Logger.getLogger(SmoothTerrainPerformer.class.getName());
    private static final int RADIUS = 10;

    @Override
    public short getActionId() {
        return (short) ExtraActionsMod.smoothTerrainId;
    }

    @Override
    public boolean action(Action action, Creature performer, int tilex, int tiley,
                          boolean onSurface, boolean corner, int tile, int heightOffset,
    ...
    public boolean action(Action action, Creature performer, Item source, int tilex, int tiley,
                          boolean onSurface, boolean corner, int tile, int heightOffset,
                          short num, float counter) {
        return action(action, performer, tilex, tiley, onSurface, corner, tile, heightOffset, num, counter);
    }

    private static void smoothArea(int tilex, int tiley) {
        ...
        if (elapsed > 500) {
            logger.info(String.format(
                    "[extraactions] smoothing terrain at [%s, %s] took %d ms",
                    tilex, tiley, elapsed));
        }
    }
